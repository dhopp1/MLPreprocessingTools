...
@testset "fill_missing" begin
    # mean, all columns
    @test isequal(fill_missing(test_data, method=mean), DataFrame(
        w = ["one", "two", "three", "four"],
        x = [1.0, 2.0, 3.0, 2.0],
    ...
    # mode, all columns
    @test isequal(fill_missing(test_data, method=median), DataFrame(
        w = ["one", "two", "three", "four"],
        x = [1.0, 2.0, 3.0, 2.0],
    ...
    @test isequal(fill_missing(test_data, [:y, :z],method=mean), DataFrame(
        w = ["one", "two", "three", "four"],
        x = [1, 2, 3, missing],
        y = [15.0, 40.0, 90.0, 15.0],
        z = [-1.0, -20.0, 20.0, -20.0]
